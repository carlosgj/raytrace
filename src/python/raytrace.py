...
class Surface(object):
    def __init__(self, name="Unnamed"):
        self.name = name
        self.logger = logging.getLogger(f"Surf::{name}")
        self.position = np.array([np.nan, np.nan, np.nan], dtype='double')
        self.direction = np.array([np.nan, np.nan, np.nan], dtype='double')
        self.type = SurfaceType.REFERENCE
        self.index = np.nan

    def setDirection(self, vec):
        self.direction = vec / np.linalg.norm(vec)
        self.logger.debug(f"Setting direction to {self.direction}")
    ...
    def setType(self, newType):
        if isinstance(newType, str):
            try:
                self.type = SurfaceType[newType]
                self.logger.debug(f"Setting surface type to {self.type}")
            except:
                raise
            ...
            self.logger.error(f"Can't use {type(newType)} to set surface type")

    def setIndex(self, index):
        self.index = index
        self.logger.debug(f"Setting index of ref. to {self.index}")
